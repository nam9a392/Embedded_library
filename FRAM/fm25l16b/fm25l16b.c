...
{
	me->meIF = meIF;
	fm25l16b_get_status(me,&me->status);
}
...
bool fm25l16b_set_status(fm25l16b_t *me, fm25l16b_status_t *status)
{
	...
	uint8_t i = 0;
	uint8_t status_reg = 0;
	fm25l16b_write_enable(me, true);
	status_reg = ((status->WEL & 0x01) << 0x01) |
			     ((status->BP0 & 0x01) << 0x02) |
				 ((status->BP1 & 0x01) << 0x03) |
				 ((status->WPEN & 0x01) << 0x07);
	fm25l16bIF_t *tmpIF = me->meIF;
	me->cmd[i++] = OP_WRSR;
	me->cmd[i++] = status_reg;
